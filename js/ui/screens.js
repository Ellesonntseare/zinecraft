...
(function () {
  ...
  PauseScreen.prototype.layout = function () {
    ...
    if (MC.Net.active) {
      var vlabel = function () { return !MC.Voice.enabled ? 'Voice: Connecting\u2026' : (MC.Voice.mode === 'open' ? 'Voice: Open Mic' : 'Voice: Push-to-Talk (V)'); };
      this.button(cx + 4, y + 80, 98, 20, vlabel, function () { if (MC.Voice.enabled) MC.Voice.setMode(MC.Voice.mode === 'open' ? 'ptt' : 'open'); }, 'Room ' + MC.Net.room + ' \u2014 click to toggle mic mode');
    } else {
  ...
  MultiplayerScreen.prototype.layout = function () {
    var g = MC.Gui, W = g.W, H = g.H, self = this; this.clear(); var cx = Math.floor(W / 2);
    var y = 44;
    this.nameField = this.field(cx - 150, y, 300, 20, this.name, { maxLen: 24, onChange: function (t) { self.name = t; } });
    this.serverField = this.field(cx - 150, y + 34, 300, 20, this.server, { maxLen: 64, onChange: function (t) { self.server = t; } });
    this.roomField = this.field(cx - 150, y + 68, 300, 20, this.room, { maxLen: 8, onChange: function (t) { self.room = t.toUpperCase(); } });
    this.seedField = this.field(cx - 150, y + 102, 145, 20, this.seed, { maxLen: 32, placeholder: 'random', onChange: function (t) { self.seed = t; } });
    this.gmBtn = this.button(cx + 5, y + 102, 145, 20, function () { return 'Mode: ' + (self.gameMode === 'creative' ? 'Creative' : 'Survival'); }, function () { self.gameMode = self.gameMode === 'survival' ? 'creative' : 'survival'; });
    this.hostBtn = this.button(cx - 150, H - 56, 145, 20, 'Host New Game', function () { self.host(); });
    this.joinBtn = this.button(cx + 5, H - 56, 145, 20, 'Join Room', function () { self.join(); });
    this.button(cx - 150, H - 30, 300, 20, 'Cancel', function () { self.game.openScreen(self.parent); });
  };
  ...
  MultiplayerScreen.prototype.host = function () {
    if (this.busy || !this.validate()) return; this.busy = true; this.setStatus('Connecting to ' + this.server + '...');
    var self = this;
    var seedStr = MC.formatSeed(this.seed); var seedInt = MC.seedToInt(seedStr);
    MC.Net.connect(this.server, { mode: 'host', room: this.room, name: this.name, seed: seedInt, gameMode: this.gameMode }, function (res) {
      self.busy = false;
      if (res.error) { self.setStatus(res.error, true); return; }
      self.setStatus('Hosting room ' + res.room + ' \u2014 share this code with friends!');
      var info = self.game.createWorld({ name: 'Multiplayer (' + res.room + ')', seed: seedStr, gameMode: self.gameMode, difficulty: 'normal', cheats: false });
      info.multiplayer = { room: res.room, server: self.server };
      self.game.saveWorldInfo(info);
      self.game.startWorld(info);
    });
  };
  MultiplayerScreen.prototype.join = function () {
    if (this.busy || !this.validate()) return;
    if (!this.room) { this.setStatus('Enter the room code your friend shared', true); return; }
    this.busy = true; this.setStatus('Connecting to ' + this.server + '...');
    var self = this;
    MC.Net.connect(this.server, { mode: 'join', room: this.room, name: this.name }, function (res) {
      self.busy = false;
      if (res.error) { self.setStatus(res.error, true); return; }
      self.setStatus('Joined room ' + res.room);
      var seedStr = MC.formatSeed(res.seed);
      var worlds = self.game.listWorlds(); var existing = null;
      for (var i = 0; i < worlds.length; i++) if (worlds[i].multiplayer && worlds[i].multiplayer.room === res.room) existing = worlds[i];
      var info = existing || self.game.createWorld({ name: 'Multiplayer (' + res.room + ')', seed: seedStr, gameMode: res.gameMode || 'survival', difficulty: 'normal', cheats: false });
      info.multiplayer = { room: res.room, server: self.server };
      self.game.saveWorldInfo(info);
      self.game.startWorld(info);
    });
  };
  MultiplayerScreen.prototype.key = function (code) { if (code === 'Enter' && !this.focus) { this.join(); return true; } return false; };
  MultiplayerScreen.prototype.render = function () {
    ...
    var cx = Math.floor(W / 2); var y = 44;
    g.text('Your Name', cx - 150, y - 10, '#a0a0a0', true);
    g.text('Server Address', cx - 150, y + 24, '#a0a0a0', true);
    g.text('Room Code (blank = create a new one when hosting)', cx - 150, y + 58, '#a0a0a0', true);
    g.text('Seed (host only)', cx - 150, y + 92, '#a0a0a0', true);
    this.renderWidgets();
    if (this.status) g.textC(this.status, cx, H - 82, this.statusColor, true);
    g.textC('Voice chat: hold V to talk once you\u2019re in. Everyone must use the same server address.', cx, H - 12, '#707070', true);
  };
