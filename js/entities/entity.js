...
(function () {
  ...
  function ItemDrop(ents, stack, pos, vel) {
    ...
    this.puppet = false; this.netId = null; this._netTarget = null;
  }
  ItemDrop.prototype.applyNetState = function (s) { this._netTarget = new THREE.Vector3(s.x, s.y, s.z); if (!this._everSet) { this.pos.copy(this._netTarget); this._everSet = true; } this.stack.count = s.count; };
  ItemDrop.prototype.update = function (dt, player, ents) {
    this.age += dt; this.pickupDelay -= dt;
    if (this.puppet) {
      var f = 1 - Math.pow(0.001, dt); if (this._netTarget) this.pos.lerp(this._netTarget, f);
      this.spin += dt * 1.5; var bob2 = Math.sin(this.age * 2) * 0.05 + 0.1;
      this.mesh.position.set(this.pos.x, this.pos.y + (this.isBlock ? 0.125 : 0.25) + bob2, this.pos.z); this.mesh.rotation.y = this.spin;
      if (this.pickupDelay <= 0 && player && !player.dead) {
        var pdx = player.pos.x - this.pos.x, pdy = (player.pos.y + 0.9) - this.pos.y, pdz = player.pos.z - this.pos.z;
        if (pdx * pdx + pdy * pdy + pdz * pdz < 1.6 * 1.6) {
          var pleft = player.inventory.add(this.stack.id, this.stack.count, this.stack.damage);
          if (pleft < this.stack.count) { MC.Audio.play('random.pop', { pitch: 0.8 + Math.random() * 0.4 }); if (player.onPickup) player.onPickup(this); MC.Net.sendPickup('drop', this.netId); this.dead = true; }
        }
      }
      return;
    }
  ...
  XPOrb.prototype.applyNetState = function (s) { this._netTarget = new THREE.Vector3(s.x, s.y, s.z); if (!this._everSet) { this.pos.copy(this._netTarget); this._everSet = true; } };
  XPOrb.prototype.update = function (dt, player, ents) {
    this.age += dt; if (this.age > 120) { this.dead = true; return; }
    if (this.puppet) {
      var f = 1 - Math.pow(0.001, dt); if (this._netTarget) this.pos.lerp(this._netTarget, f);
      this.mesh.position.set(this.pos.x, this.pos.y + 0.15, this.pos.z); if (player) this.mesh.lookAt(player.getEyePos());
      var hue2 = (this.age * 2) % 1; this.mesh.material.color.setHSL(0.25 + hue2 * 0.12, 1, 0.6);
      if (player && !player.dead) { var xdx = player.pos.x - this.pos.x, xdy = player.pos.y + 0.9 - this.pos.y, xdz = player.pos.z - this.pos.z; var xd = Math.sqrt(xdx * xdx + xdy * xdy + xdz * xdz); if (xd < 0.9 && this.age > 0.4) { player.addXP(this.value); MC.Audio.play('random.orb', { pitch: 0.9 + Math.random() * 0.3 }); this.dead = true; MC.Net.sendPickup('xp', this.netId); } }
      return;
    }
