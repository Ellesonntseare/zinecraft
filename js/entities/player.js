...
(function () {
  ...
  Player.prototype.updateInteraction = function (dt, input) {
    ...
    if (left && this.targetMob && this.attackCooldown <= 0) {
      ...
      var knock = this.sprinting ? 0.9 : 0.5;
      if (this.targetMob.puppet) { MC.Net.sendMobHit(this.targetMob.netId, dmg, this.pos, knock); this.targetMob.hurtT = 0.5; this.targetMob.material.uniforms.uHurt.value = 1; }
      else this.targetMob.hurt(dmg, 'player', this.pos, knock);
    ...
    if (right) {
      ...
      if (pressedNow && this.targetMob && MC.Mobs.interact(this.targetMob, this, held)) {
        if (this.targetMob.puppet) MC.Net.sendMobInteract(this.targetMob.netId, held.id === 'shears' ? 'shear' : (held.id === 'bucket' ? 'milk' : 'feed'));
        this.swingArm(); this.useCooldown = 0.25; return;
      }
