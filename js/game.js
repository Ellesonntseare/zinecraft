...
(function () {
  ...
  Game.prototype.toTitle = function () {
    ...
    MC.Net.disconnect(); MC.Net.game = null;
  ...
  Game.prototype.startWorld = function (info) {
    ...
    MC.Net.game = this;
    if (info.multiplayer && MC.Net.active) { var self2 = this; setTimeout(function () { if (self2.chat) self2.chat.add('\u00a77Connected to room ' + info.multiplayer.room + '. Hold V to talk.'); }, 50); }
  ...
  Game.prototype.finishLoading = function () {
    ...
    var joiningPeer = MC.Net.active && !MC.Net.isHost;
    if (data && data.player) { this.player.load(data.player); if (!joiningPeer) MC.Mobs.load(data.mobs); }
    else { var sp = this.world.findSpawn(); this.player.pos.copy(sp); this.player.prevPos.copy(sp); this.player.spawn.copy(sp); if (!joiningPeer) { MC.Mobs.seedSalt = this.worldInfo.seedInt; MC.Mobs.initialSpawn(sp, 10); } }
    ...
    if (this.worldInfo.multiplayer && MC.Net.active) {
      MC.Voice.start().catch(function (err) { self.chat.add('\u00a7cMicrophone unavailable (' + (err && err.name || 'error') + ') \u2014 you can still play and see others, just not talk.'); });
    }
  };
  ...
  Game.prototype.onBlockChanged = function (e) {
    if (MC.Net.active && !MC.Net.applyingRemote) MC.Net.sendBlock(e.x, e.y, e.z, e.id, this.world.getMeta(e.x, e.y, e.z));
  ...
  Game.prototype.spawnDrop = function (stack, pos, vel, pickupDelay) {
    if (!stack || !MC.ITEMS[stack.id]) return;
    if (MC.Net.active && !MC.Net.isHost) { MC.Net.requestDrop(stack, pos, vel, pickupDelay); return; }
    var d = new MC.ItemDrop(this.entities, { id: stack.id, count: stack.count, damage: stack.damage || 0 }, pos, vel);
    if (pickupDelay !== undefined) d.pickupDelay = pickupDelay;
    if (MC.Net.active) d.netId = MC.Net.nextId();
    this.entities.add(d);
    return d;
  };
  Game.prototype.dropItem = function (stack) { if (!stack) return; var p = this.player; var d = p.getLookDir(); var pos = p.getEyePos(1).add(d.clone().multiplyScalar(0.3)); pos.y -= 0.3; var vel = d.clone().multiplyScalar(6); vel.y += 1; vel.x += (Math.random() - 0.5); vel.z += (Math.random() - 0.5); this.spawnDrop(stack, pos, vel, 2); };
  Game.prototype.spawnXP = function (pos, n) {
    if (MC.Net.active && !MC.Net.isHost) { MC.Net.requestXP(pos, n); return; }
    while (n > 0) { var v = Math.min(n, n > 10 ? 7 : 3); var orb = new MC.XPOrb(this.entities, pos, v); if (MC.Net.active) orb.netId = MC.Net.nextId(); this.entities.add(orb); n -= v; }
  };
  ...
  Game.prototype.updatePlaying = function (dt) {
    ...
    if (!paused) {
      ...
      MC.Net.tick(dt, p);
      if (MC.Voice.enabled) { var talkKey = !this.chat.open && !this.screen && input.down('KeyV'); MC.Voice.setTalking(talkKey); MC.Voice.tick(); }
    }
